...
func TestReElection2A(t *testing.T) {
	...
	t.Logf("leader is %v", leader1)
	t.Logf("Disconnecting leader %v", leader1)
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	l := cfg.checkOneLeader()
	t.Logf("Checked New Leader %v", l)
	// if the old leader rejoins, that shouldn't
	// disturb the new leader. and the old leader
	// should switch to follower.
	t.Logf("Reconnecting old leader %v", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	t.Logf("Checked New Leader %v", leader2)
	// if there's no quorum, no new leader should
	// be elected.
	t.Logf("Disconnect leader %v", leader2)
	cfg.disconnect(leader2)
	t.Logf("Disconnect server %v", (leader2+1)%servers)
